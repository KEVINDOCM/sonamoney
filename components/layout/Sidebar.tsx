...
export function Sidebar({ budgetWarningCount, children }: SidebarProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col lg:flex-row">
      ...
      <aside className="group fixed inset-y-0 left-0 z-40 w-16 hover:w-64 bg-white dark:bg-slate-900 hidden lg:flex flex-col py-6 transition-all duration-300 ease-out overflow-hidden border-r border-slate-200 dark:border-slate-800 shadow-[2px_0_8px_rgba(0,0,0,0.04)]">
        {/* Logo section */}
        <div className="mb-8 px-3 flex items-center gap-3 overflow-hidden">
          <Link href="/dashboard" aria-label="SonaMoney Home" className="flex items-center gap-3">
            <div className="h-9 w-9 shrink-0 rounded-xl bg-gradient-to-br from-teal-500 to-teal-600 flex items-center justify-center text-white font-bold text-lg shadow-teal-sm">
              S
            </div>
            <span className="font-bold text-slate-900 dark:text-white whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200">
              SonaMoney
            </span>
          </Link>
        </div>
